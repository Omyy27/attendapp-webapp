      ...
      <div className="max-w-md md:max-w-2xl mx-auto min-h-screen">
        ...
        <header className="sticky top-0 z-30 bg-card/90 backdrop-blur-md px-5 md:px-8 pt-5 pb-4 border-b border-line-strong/70">
          <div className="flex items-center gap-3 mb-4">
            ...
            <div>
              ...
              <h1 className="font-serif text-lg leading-none text-content">Buscar por nombre</h1>
            </div>
          </div>
          ...
          <div className="relative">
            ...
            <input
              ...
              className="w-full bg-field border border-line-strong rounded-xl pl-11 pr-10 py-3.5 text-base text-content placeholder:text-muted-soft focus:outline-none focus:ring-2 focus:ring-sky/40 focus:border-sky/40"
            />
            {query && (
              <button
                onClick={() => { setQuery(""); setResults([]); }}
                className="absolute right-3.5 top-1/2 -translate-y-1/2 text-muted-soft hover:text-content-soft"
              >
        ...
        <section className="px-5 md:px-8 pt-4">
          <div className="flex items-start gap-2.5 bg-sky/5 border border-sky/20 rounded-xl p-3.5">
            ...
            <p className="text-xs text-content-soft leading-relaxed">
        ...
        <section className="px-5 md:px-8 pt-4">
          ...
          {query.length >= 2 && !searching && results.length === 0 && (
            <div className="text-center py-10">
              <p className="text-sm text-muted">No se encontraron invitados</p>
              <p className="text-xs text-muted-soft mt-1">Intenta con otro nombre</p>
            </div>
          )}

          {query.length < 2 && (
            <div className="text-center py-10">
              <p className="text-sm text-muted-soft">Escribe al menos 2 letras para buscar</p>
            </div>
          )}
        </section>
